...
\[ \text{conv}(S) := \bigg\{ \sum_{i=1}^{N} \lambda_{i} \vx_{i} \mid \vx_{i} \in S, \lambda \geq 0, \sum_{i=1}^{N} \lambda_{i} = 1 \bigg\} . \]
 ...
 \begin{enumerate}
 \item Prove that $\text{conv}(S) = \cap_{K \supseteq S} K$ where the intersection is over all closed convex $K$ containing $S$. Therefore $\text{conv}(S)$ is the smallest convex set containing $S$. 
...
\begin{itemize}
    \item Show MEM$(\text{epi}(f))$ can be implemented using an EVALuation oracle for $f$; show EVAL$(f)$ can be approximately implemented using MEM$(\text{epi})$, i.e. for input $x \in \R^{n}$ compute $t \in \R$ such that $|t - f(x)| \leq \eps$. 
    ...
    \item Relate OPT$(\text{epi}(f))$ and EVAL and GRAD for the Fenchel dual defined as
    \[ f^{*}(w) := \sup_{x \in \R^{n}} \langle w, x \rangle - f(x) . \]

    \item Relate MEM, SEP for $\text{epi}(f^{*})$ with VAL, VIOL for $\text{epi}(f)$. Note the relation does not need to be exact.  
\end{itemize}
